...
/* MiGroBox ESP32s2 stepper motor control code header - ECE Capstone Fall 2020 
    - Controls stepper drivers
*/


/**
 * @brief Type defined for stepper motor handle
 *
 */
...
TaskHandle_t y_endstop_yHandle = NULL; 

//static xQueueHandle gpio_evt_queue = NULL;

static void IRAM_ATTR z_endstop_isr_handler(void* arg){
    uint32_t gpio_num = (uint32_t) arg;
    //xQueueGenericSendFromISR(gpio_evt_quque, &gpio_num, NULL)
    //xQueueSendFromISR(gpio_evt_queue, &gpio_num, NULL);
    xTaskResumeFromISR(z_endstop_xHandle);
}
/*
...
stepper_t create_stepper(gpio_num_t gpio){   
